...
import { listTransferRuns } from '../../../lib/process-runs-store.js';
  ...
  try {
    ...
    const status =
      statusParam === 'active' || statusParam === 'completed' || statusParam === 'canceled'
        ? statusParam
        : undefined;
    const runs = await listTransferRuns({
    ...
    console.error('Failed to load transfer runs:', error);
    return res.status(500).json({
      success: false,
      error: 'Failed to load transfer runs.',
    });
  }
}
