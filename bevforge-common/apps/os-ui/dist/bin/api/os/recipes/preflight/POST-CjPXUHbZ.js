...
import { j as checkInventoryForRecipe } from '../../../calendar/events/GET-DNBekL63.js';

const nowIso = () => (/* @__PURE__ */ new Date()).toISOString();
const normalizeText = (value) => String(value ?? "").trim().toLowerCase();
const normalizeTargetId = (value) => normalizeText(value).replace(/[^a-z0-9]+/g, "_").replace(/^_+|_+$/g, "");
const nodeDeviceId = (node) => node.data.logicalDeviceId ?? node.id;
const nodeLabel = (node) => `${node.data.label} (${node.data.widgetType})`;
const addRequirementMessage = (requirement, blockers, warnings) => {
  if (requirement.outcome === "met") return;
  const base = `${requirement.label}: ${requirement.detail}`;
  const withFallback = requirement.manualFallback ? `${base} Fallback: ${requirement.manualFallback}` : base;
...
const inferRecipeStages = (recipe) => {
...
const includesAny = (haystack, needles) => haystack.some((value) => needles.some((needle) => value.includes(needle)));
const buildRecipePreflightReport = (recipe, project, devices, equipmentRoleMap, inventoryChecks = []) => {
  ...
  const nodesByType = (widgetType) => nodes.filter((node) => node.data.widgetType === widgetType);
  ...
  const vesselByType = (vesselType) => vesselNodes.filter(
  ...
  const nodeByAnyId = /* @__PURE__ */ new Map();
  nodes.forEach((node) => {
  ...
  const mappedNodeIds = (...roles) => roles.map((role) => resolveMappedRole(role)).flatMap((entry) => {
  ...
  const mappedLabels = (...roles) => roles.map((role) => resolveMappedRole(role)).flatMap((entry) => {
  ...
  const stepTexts = recipe.steps.map(
    (step) => `${normalizeText(step.stage)} ${normalizeText(step.action)} ${normalizeText(step.name)}`
  );
  ...
  const hasCooling = stages.includes("cooling") || includesAny(stepTexts, ["cool", "chill", "cold crash", "glycol"]);
  const hasHeating = hasMash || hasBoil || includesAny(stepTexts, ["heat", "strike", "hlt", "hold_temp", "set_value"]);
  const requirements = [];
  ...
  if (nodes.length === 0) {
    addRequirement({
      ...
      manualFallback: "Add and publish your main brewhouse devices on canvas before starting recipe execution.",
      matchedDeviceIds: [],
      matchedLabels: []
    });
  }
  if (source === "all_pages" && (project.pages ?? []).length > 0) {
    addRequirement({
      ...
      manualFallback: "Publish the production pages so recipe execution always resolves against locked layouts.",
      matchedDeviceIds: [],
      matchedLabels: []
    });
  }
  if (hasMash) {
    ...
    if (mappedMashTun.configuredValue) {
      ...
      addRequirement({
        ...
        missingSeverity: "warning",
        manualFallback: "Assign one vessel as mash tun in widget config, or run mash steps manually.",
    ...
    if (mappedHlt.configuredValue) {
      ...
      if (mappedHeatSource.resolvedNode || mappedHeatSource.resolvedDevice) {
        addRequirement({
          ...
          missingSeverity: "warning",
          manualFallback: "No HLT mapped; confirm process is compatible with heater-direct operations.",
      ...
      addRequirement({
        ...
        missingSeverity: "warning",
        manualFallback: "You can heat manually in mash tun/kettle and continue with override acknowledgement.",
      ...
      addRequirement({
        ...
        missingSeverity: "blocker",
        manualFallback: "Install/configure a heater path or run heating outside BevForge and proceed manually.",
        matchedDeviceIds: [],
        matchedLabels: []
      });
    }
    ...
    if (mappedKettle.configuredValue) {
      if (mappedKettle.resolvedNode || mappedKettle.resolvedDevice) {
        ...
        addRequirement({
          ...
          detail: hasMappedHeat ? `Mapped boil kettle and heat source detected.` : `Mapped boil kettle detected, but heat source role is not mapped.`,
          missingSeverity: hasMappedHeat ? void 0 : "warning",
          manualFallback: hasMappedHeat ? void 0 : "Map `heat_source_primary` or run heating manually and confirm steps.",
          matchedDeviceIds: [
      ...
      addRequirement({
        ...
        missingSeverity: "warning",
        manualFallback: "You can boil manually, then confirm/advance recipe steps from the execution page.",
        matchedDeviceIds: vesselNodes.map(nodeDeviceId),
        matchedLabels: vesselNodes.map(nodeLabel)
      });
    } else {
  ...
  if (hasFermentation) {
    ...
    if (mappedFermenter.configuredValue) {
      ...
      addRequirement({
        ...
        missingSeverity: "warning",
        manualFallback: "Use a generic vessel and verify fermentation controls manually.",
        matchedDeviceIds: vesselNodes.map(nodeDeviceId),
        matchedLabels: vesselNodes.map(nodeLabel)
      });
    } else {
    ...
    if (mappedTransferPump.configuredValue) {
      if (mappedTransferPump.resolvedNode || mappedTransferPump.resolvedDevice) {
        ...
        addRequirement({
          ...
          missingSeverity: "warning",
          manualFallback: "Update mapping, or continue with manual transfer and confirm each step.",
          matchedDeviceIds: [],
          matchedLabels: []
        });
      }
      ...
      addRequirement({
        ...
        missingSeverity: "warning",
        manualFallback: "Use gravity/manual transfer and confirm the corresponding recipe steps.",
        matchedDeviceIds: [],
        matchedLabels: []
      });
    }
  ...
  if (hasCooling) {
    ...
    if (mappedGlycolPump.configuredValue || mappedGlycolValve.configuredValue) {
      const hasResolved = Boolean(
        mappedGlycolPump.resolvedNode || mappedGlycolPump.resolvedDevice || mappedGlycolValve.resolvedNode || mappedGlycolValve.resolvedDevice
      );
      addRequirement({
        id: "cooling-control",
        label: "Cooling Control",
        outcome: hasResolved ? "met" : "fallback",
        detail: hasResolved ? "Mapped glycol cooling role(s) detected." : "Mapped glycol role(s) are not resolved to installed equipment.",
        missingSeverity: hasResolved ? void 0 : "warning",
        manualFallback: hasResolved ? void 0 : "Update glycol role mapping or run cooling manually.",
        matchedDeviceIds: [
      ...
      addRequirement({
        ...
        missingSeverity: "warning",
        manualFallback: "Run cooling manually or build a glycol controller widget for closed-loop control.",
      ...
      addRequirement({
        ...
        missingSeverity: "warning",
        manualFallback: "Manual cold-crash/cooling is possible; confirm each step during execution.",
        matchedDeviceIds: [],
        matchedLabels: []
      });
    }
  ...
  if (hasHeating || hasCooling || hasFermentation) {
    ...
    if (mappedMashTemp.configuredValue || mappedFermentTemp.configuredValue) {
      const hasResolved = Boolean(
        mappedMashTemp.resolvedNode || mappedMashTemp.resolvedDevice || mappedFermentTemp.resolvedNode || mappedFermentTemp.resolvedDevice
      );
      addRequirement({
        id: "temperature-feedback",
        label: "Temperature Feedback",
        outcome: hasResolved ? "met" : "fallback",
        detail: hasResolved ? "Mapped temperature sensor role(s) detected." : "Mapped temperature sensor role(s) are unresolved.",
        missingSeverity: hasResolved ? void 0 : "warning",
        manualFallback: hasResolved ? void 0 : "Update temperature sensor role mapping or monitor temperature manually.",
        matchedDeviceIds: [
      ...
      addRequirement({
        ...
        missingSeverity: "warning",
        manualFallback: "You can use manual thermometer readings and adjust step values on the fly.",
        matchedDeviceIds: [],
        matchedLabels: []
      });
    }
  ...
  const aliasResolvesToMappedEquipment = (targetId) => {
  ...
  const missingTargetDevices = recipe.steps.map((step) => step.targetDeviceId).filter((targetId) => Boolean(targetId)).filter((targetId) => !knownDeviceIds.has(targetId)).filter((targetId) => !aliasResolvesToMappedEquipment(targetId));
  if (missingTargetDevices.length > 0) {
    addRequirement({
      ...
      missingSeverity: "warning",
      manualFallback: "Map missing targets in canvas/config, or run those steps manually and confirm progression.",
      matchedDeviceIds: [],
      matchedLabels: []
    });
  }
  ...
  requirements.forEach(
    (requirement) => addRequirementMessage(requirement, blockers, warnings)
  );
  const dedupedMissingTargets = [...new Set(missingTargetDevices)];
  if (dedupedMissingTargets.length > 0) {
    warnings.push(
      `Unmapped target device IDs: ${dedupedMissingTargets.slice(0, 6).join(", ")}${dedupedMissingTargets.length > 6 ? "..." : ""}`
    );
  }
  ...
  if (missingInventory.length > 0) {
    warnings.push(
      `Inventory missing for required inputs: ${missingInventory.slice(0, 6).map((item) => item.requirementName).join(", ")}${missingInventory.length > 6 ? "..." : ""}`
    );
  }
  if (lowInventory.length > 0) {
    warnings.push(
      `Low inventory for: ${lowInventory.slice(0, 6).map((item) => item.requirementName).join(", ")}${lowInventory.length > 6 ? "..." : ""}`
    );
  }
  const requiresManualOverride = requirements.some(
    (requirement) => requirement.outcome === "fallback" && requirement.id !== "published-pages" && requirement.id !== "step-target-mapping"
  ) || inventoryOverrideRequired;
  const status = blockers.length > 0 ? "incompatible" : requiresManualOverride ? "needs_override" : "compatible";
  return {
    recipeId: recipe.id,
...
async function handler(req, res) {
  try {
    const { recipeId, siteId } = req.body;
    if (!recipeId) {
...
export { buildRecipePreflightReport as b, handler as h };
