import { d as db, k as deviceTiles, t as tileEndpointBindings, f as deviceGroups, e as endpointCurrent } from '../../../../index-BiQ9ukMS.js';
...
async function handler(req, res) {
  try {
    ...
    const bindings = await db.select().from(tileEndpointBindings).where(eq(tileEndpointBindings.tileId, tileIdNum));
    const children = await db.select().from(deviceTiles).where(eq(deviceTiles.parentTileId, tileIdNum));
    let group = null;
    if (tile.groupId) {
      const [groupRow] = await db.select().from(deviceGroups).where(eq(deviceGroups.id, tile.groupId));
      group = groupRow || null;
    }
    const response = {
      tile: {
