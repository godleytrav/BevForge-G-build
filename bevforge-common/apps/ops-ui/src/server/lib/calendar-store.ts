...
const defaultFeed = (suite: CalendarSuiteId) => ({
  schemaVersion: '1.0.0',
...
const normalizeEvent = (
  ...
  const linksRaw =
    raw.links && typeof raw.links === 'object'
      ...
      : undefined;
  const openPath = toText(linksRaw?.openPath ?? linksRaw?.open_path);
  const openUrl = toText(linksRaw?.openUrl ?? linksRaw?.open_url);
...
const buildOsDerivedEvents = async (): Promise<CalendarEventRecord[]> => {
  const recipeRuns = await readJsonOrDefault<{ runs?: RecipeRunRecord[] }>(
    path.join(commissioningRoot, 'os', 'recipe-runs.json'),
    { runs: [] }
  );
  const batchState = await readJsonOrDefault<{ batches?: BatchRecord[] }>(
    path.join(commissioningRoot, 'os', 'batch-state.json'),
    { batches: [] }
  );
